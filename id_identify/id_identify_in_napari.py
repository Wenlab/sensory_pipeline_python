...
def image_and_box_load(green_file_path, red_file_path,  neuron_pt_tuple_path , root_volume_path = None, root_neuron_pt_tuple = None):
    """
    Args:
        ...
        root_volume_path (_type_, optional): aligned MIP volume 
        root_neuron_pt_tuple (_type_, optional): neuron pt tuple for aligned MIP volume
    Returns:
    ...
    if root_volume_path is not None:
        root_volume = np.load(root_volume_path)
        green[1] = root_volume.transpose(2,0,1)
        if root_neuron_pt_tuple is not None:
            neuron_pt_tuple[1] = np.load(root_neuron_pt_tuple)
...
def id_identify_in_napari(green_file_path, red_file_path,  neuron_pt_tuple_path , aligned_volume_path = None, mask_name = "mask",green_scale=[1,5,1,-1], red_scale = [1,5,1,1], red_translate = [0,0,0,0], green_translate = [0,0,0,1024], mask_scale=[1,5,1,-1], mask_translate=[0,0,0,1024]):
    ...
    green, red, mask = image_and_box_load(green_file_path=green_file_path,
                                          ...
                                          root_volume_path=aligned_volume_path)
...
# %%
if __name__ == "__main__":
    green_file_path = r"H:\Process_temporary\WJH\olfactory\ID\image_data\20250712\w1\green.npy"
    red_file_path = r"H:\Process_temporary\WJH\olfactory\ID\image_data\20250712\w1\red.npy"
    aligned_volume_path = r"H:\Process_temporary\WJH\olfactory\ID\image_data\20250712\w1\synthetic_volume\aligned_volumes_mip.npy"
    neuron_pt_tuple_path = r"H:\Process_temporary\WJH\olfactory\ID\image_data\20250712\w1\synthetic_volume\all_neuron_pt_tuple.npy"
