...
def load_intensity_and_dID(h5_file_path, root_name=None):
    ...
    try:
        ...
        dID = np.arange(intensity.shape[0])
    try:
        n_seq = data['n_seg'][:]
    except KeyError:
        n_seq = np.array([])
    return intensity, dID, n_seq
...
def draw_raw_signal(h5_file_path, save_folder, exp_name, root_name = None, 
    ...
    intensity, dID, n_seq = load_intensity_and_dID(h5_file_path, root_name=root_name)
    neuron_ids = dID
...
def draw_trend_signal(h5_file_path, save_folder, exp_name, root_name=None,
    ...
    if labjack_excel_path:
        ...
        delta_F_over_F0, fitted_F0, quality_info = calculate_delta_F_over_F0(
            ...
            baseline_pre=baseline_pre,
            n_seq=n_seq,
        )  # delta_F_over_F0 is a dataframe
...
if __name__ == "__main__":
    intensity_path = r"H:\Process_temporary\WJH\olfactory\infer_result\20251105\20251105.h5"
    import h5py
    from result_plot.draw_mean_signal import *

    with h5py.File(intensity_path, "r") as f:
        key_list = list(f.keys())
    labjack_excel_path = (
        r"H:\Process_temporary\WJH\olfactory\labjack_result\20251105_bac\output_volumes_merged.xlsx"
    )
    bio_ID_path = None
    for key in key_list:
        save_folder = rf"H:\Process_temporary\WJH\olfactory\plot\raw_image\20251104\{key}"
        trend_args = {
            "h5_file_path": intensity_path,
            "save_folder": save_folder,
            "exp_name": f"{key}",
            "root_name": key,
            "date": "20251104_bacteria",
            "bi_ID_path": bio_ID_path,
            "odor_config_file": r"H:\Process_temporary\WJH\sensory_pipeline_python\data_load\config\bacteria.json",
            "stimulus_color_path": r"H:\Process_temporary\WJH\sensory_pipeline_python\data_load\config\bacteria_color_scheme.json",
